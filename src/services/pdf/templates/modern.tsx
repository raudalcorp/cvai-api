// src/services/pdf/templates/modern.tsx
// Plantilla "Moderno" — barra lateral de color con contacto,
// contenido principal en columna derecha. Ideal para tech/creativo.
...
const s = StyleSheet.create({
  ...
  sSkillChip: { backgroundColor: 'rgba(255,255,255,0.1)', padding: '3pt 8pt', borderRadius: 8, fontSize: 7.5, marginBottom: 4, marginRight: 4 },
  ...
  skillChip:  { backgroundColor: C.accentLt, color: C.accent, fontSize: 7.5, padding: '2pt 7pt', borderRadius: 8 },
...
export function ModernTemplate({ data }: { data: CvFormData }) {
  // Collect all unique skills from experience
  const allSkills = [...new Set(data.experience.flatMap((e) => e.skills))].slice(0, 12)
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.sidebar}>
          ...
          {data.jobTitle && <Text style={s.sJobTitle}>{data.jobTitle}</Text>}
          ...
          {allSkills.length > 0 && (
            <>
              ...
              <View style={s.sSkillsRow}>
                {allSkills.map((sk) => <Text key={sk} style={s.sSkillChip}>{sk}</Text>)}
              </View>
            </>
          )}
              ...
              {data.experience.map((exp) => (
                <View key={exp.id} style={{ marginBottom: 10 }}>
                  ...
                  <Text style={s.expMeta}>
                    {exp.company}{exp.city ? ` · ${exp.city}` : ''} · {formatPeriod(exp.period.startYear, exp.period.startMonth, exp.period.endYear, exp.period.endMonth)}
                  </Text>
